...
h <- ggplot(full_df, aes(x=LSfact, y=tss_load, color=Treatment), inherit.aes = FALSE) +
  geom_point() + 
  scale_y_log10() +
...
g <- ggplot(full_df, aes(x=LSfact, y=log10(tss_load+0.0053), color=Treatment), inherit.aes = FALSE) +
  geom_point() + 
  geom_smooth(method=lm, se=TRUE, fullrange=TRUE) +
  #scale_y_log10() +
  ggtitle("Log-log relationship between TSS load and rainfall accumulation \n(full dataset)") +
...
m_flume <- lmerTest::lmer(log(tss_load+0.0053) ~ 
                            ...
                            #(1 | SiteID) +
                             (1 | SiteID:Treatment) + 
                            (1|Year:sample_event) + #consider this and above with SiteID 
                            (1|SiteID:Year:sample_event), #removed due to singular fit
...
```{r design_model_figure, dependson="design_model_contrasts"}
