...
h <- ggplot(full_df, aes(x=log(ppt_sum), y=log(tss_load+0.0016), color=Treatment), inherit.aes = FALSE) +
 scale_color_manual(values = c("control" = "#C38820",
                    ...
                    name  ="") +
 geom_point(size=4) +
 xlab("log(Rainfall accumulation (mm))") +
 ylab("log(TSS load (kg/ha))") +
 geom_smooth(method=lm, se=FALSE, fullrange=TRUE) +
 ggtitle("A. Full Dataset") +
 theme(plot.title = element_text(family="A", size=22, face="bold", hjust=0.5),
       axis.title.x = element_text(family="A", size=22, face="bold"),
       axis.title.y = element_text(family="A", size=22, face="bold"),
       axis.text.x = element_text(family="A", size=22),
       axis.text.y = element_text(family="A", size=22),
...
m2_flume <- lmerTest::lmer(log(tss_load+0.0016488035) ~ 
                             trt_lnppt +
                            Treatment*crop +
                            Year*Treatment +
...
#plot(pairs(emmeans(m_flume, 'Treatment', 'crop'), adjust="jessica")) + 
...
trt_plot <- group %>%
  ...
  xlab("Treatment") +
  ylab("TSS Load (kg/ha)") +
 theme(plot.title = element_text(family="A", size=18, face="bold", hjust=0.5),
       axis.title.x = element_text(family="A", size=18, face="bold"),
       axis.title.y = element_text(family="A", size=18, face="bold"),
       axis.text.x = element_text(family="A", size=18),
       axis.text.y = element_text(family="A", size=18)) +
