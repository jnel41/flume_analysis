...
mR_flume <- lmerTest::lmer(log(tss_load+0.0016488035) ~ 
                             Treatment*ln_ppt +
                            Year*Treatment +
...
h <- ggplot(flumeR, aes(x=log(ppt_sum), y=log(tss_load+0.0016488035), color=Treatment), inherit.aes = FALSE) +
  scale_color_manual(values = c("control" = "#176D9C",
                               "strips" = "#C38820")) +
  geom_point(aes(shape=SiteID), size=4) + 
  #scale_y_log10() +
